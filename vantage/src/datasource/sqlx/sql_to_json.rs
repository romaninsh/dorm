// source from https://github.com/sqlpage/SQLPage/blob/a8f6d338bb3807f2bf23c75752c0be20bbec233c/src/utils.rs#L4
...
fn decode_raw<'a, T: Decode<'a, sqlx::Postgres> + Default>(
    raw_value: sqlx::postgres::PgValueRef<'a>,
) -> T {
...
pub fn row_to_string(row: &PgRow) -> Option<String> {
...
mod tests {
    ...
    async fn test_row_to_json() -> anyhow::Result<()> {
        use sqlx::Connection;
        let db_url = test_database_url();
        let mut c = sqlx::PgConnection::connect(&db_url).await?;
        let row = sqlx::query(
            "SELECT \
